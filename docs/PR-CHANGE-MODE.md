...
The toolbar offers three views, remembered across reloads: Pseudo (pseudocode alone, no source pane rendered), Pseudo + unified and Pseudo + split. The adjacent TypeScript pane uses Pierre `FileDiff` on the extracted source functions. A registered Pierre theme derives its token colors from the same selected palette as the pseudocode while preserving Pierre's line and intra-line diff markup. The remainder uses exact file diffs. Test, eval, fixture and test-helper paths are excluded before report data is assembled.
