...
```text
envelope = telemetryBody.parse(payload)
// { source: "splunk", payload: alert }
input = parseSplunkSignal(envelope.payload, receivedAt)
```

- `source: "splunk"` duplicates the source identity already conveyed by `parseSplunkSignal`.
- `payload: alert` duplicates the nesting already conveyed by `envelope.payload`. The placeholder `alert` adds neither structure nor a concrete value.
...
```text
severity = SPLUNK_SEVERITY[sourceSeverity] // "high" → ERROR
```

Accept: the lookup expression does not expose the table entry. The annotation connects the incoming value to the normalized severity. Do not flag it merely because the source table could be opened elsewhere.

### A value connects distant stages

```text
hostRaw = payload.result.host // "db-01"
```

Accept when the slice follows db-01 into host lookup and later work: the value helps track the same data across components. It need not affect a branch to be useful. If the exact value is already visible next to this line, assess that local duplication instead of banning example values globally.

### A partial object explains the next consumer

```text
recorded = Signal.record(input) → {signal, inserted: true, ...}
...
```text
Signal.record(input, receivedAt)
```

Accept for an ordinary new Signal().record(...) invocation under the language's instance shorthand. Do not demand new, await, logging, query scope, or every fallback simply because they appear in the source. Flag an omission only if it hides a meaningful dependency, transformation, ordering requirement, or outcome in the selected slice.

### A meaningful transformation was lost

If source fingerprints a cleaned and truncated exemplar but pseudocode fingerprints the original text, flag the mismatch: it changes which data determines identity. Request the missing transformation; do not request all unrelated parsing branches.
...
`No cached or stored Sentinel workflow; enqueue succeeds` is useful because it explains why both existing-workflow exits and the failure path are absent. `Registers Sentinel successfully` adds little beyond the function name and result. Do not demand that every trivial precondition appear in the caption.
