...
| Slice instructions | Which execution and level of detail are we explaining? | One selected execution from its trigger to closure; simplest scenario; review order by data flow or dependency; no scenario forks or obvious assumptions; code-centered orchestration across components | [SLICE-INSTRUCTIONS.md](SLICE-INSTRUCTIONS.md) |
