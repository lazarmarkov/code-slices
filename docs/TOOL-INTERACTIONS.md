# Execution-slice report interactions

These rules cover how the execution-slice report looks and responds, not what the pseudocode means or which scenario it follows. PR-change reports are described in [PR-CHANGE-MODE.md](PR-CHANGE-MODE.md).
...
- Use concrete artifact names, such as order-to-delivery.call-tree.txt, rather than happy-path.call-tree.txt.
...
The current prototype (`src/runtime.js`) does not yet do the following:

- Source reference links for references outside the displayed function, their place in the sticky footer, and the Source reference label.
- References into other functions or files in the floating source excerpt.
- The button fallback for Alt type expansion.
- Function-level navigation.
- Signatures on one line with horizontal scrolling: the renderer wraps long lines, so a long signature wraps.
