...
- A parameter or field that may be `null` or `undefined` is written with `?` instead of the union: `hostRaw?` for `hostRaw?: string | null` and `hostRef?: HostRef` for `hostRef: HostRef | null`. Return types keep the explicit union, since returning `null` is an outcome the reader must see.
