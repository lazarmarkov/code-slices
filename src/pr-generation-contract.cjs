...
- Omit a parameter or field annotation whose whole type is string, number or bool. Write a parameter or field that may be null or undefined with ? instead of the union (hostRaw? for hostRaw?: string | null, hostRef?: HostRef for hostRef: HostRef | null). Keep other unions, arrays, generics, named types and every return type. Use bool. Predicate aliases may end in ?. Use .in?(a, b) for membership; status(a | b) only lists return variants. map(field) is field projection. a..b is inclusive. Express negated guards with if !condition. Use postfix if guards only when they preserve the source guard. Keep explicit assignment and do not use implicit returns or calls.`;

const scopeIdentifierPattern = /\b(?:org(?:anization)?_?ids?|tenant_?ids?|allowedOrgIds)\b/gi;
