...
const makeFile=(container,name,contents,overflow=options.overflow)=>new File({...options,overflow}).render({containerWrapper:container,file:{name,contents}});
const makeDiff=(container,name,before,after,overflow=options.overflow)=>new FileDiff({...options,overflow,diffStyle:'split'}).render({containerWrapper:container,oldFile:{name,contents:before||''},newFile:{name,contents:after||''}});
// A diffs container fills its pane instead of sizing to its text, so the pseudocode column is sized from the longest lines on both sides; the source scrolls rather than wraps.
const codeWidth=(texts,columns)=>{const lines=texts.filter(t=>t!=null).flatMap(t=>t.split('\n'));if(!lines.length)return '50%';const longest=Math.max(...lines.map(line=>line.length));return 'calc('+columns+' * ('+longest+' * var(--code-char) + var(--code-gutter)))';};
...
function renderFlows(){
 ...
 const panes=card.querySelector('.panes'),changes=state.revision==='changes';panes.style.setProperty('--pseudo-need',codeWidth(changes?[c.before,c.after]:[pseudo],changes?2:1));panes.style.setProperty('--source-need',codeWidth(changes?[c.sourceBefore?.code,c.sourceAfter.code]:[source?.code],changes?2:1));
 const pseudoNode=card.querySelector('.pseudo-code'),sourceNode=card.querySelector('.source-code');
 if(state.revision==='changes'){makeDiff(pseudoNode,c.symbol+'.pseudo.ts',c.before,c.after);makeDiff(sourceNode,c.file,c.sourceBefore?.code,c.sourceAfter.code,'scroll');}
 ...
 else {makeFile(pseudoNode,c.symbol+'.pseudo.ts','▹ '+(rev==='before'?(c.scenarioBefore||c.scenario):c.scenario)+'\n'+pseudo);makeFile(sourceNode,c.file,source.code,'scroll');requestAnimationFrame(()=>attachPair(card,source,rev==='before'?c.mappingsBefore:c.mappingsAfter));}
 }
 }
